...
export interface JsonLdInput {
  ...
  /** All locales the site supports (used for availableLanguage on WebSite node) */
  supportedLocales?: string[];
  /** Organization config from WebsiteConfig */
  organization?: OrganizationInfo;
  /** Page-specific data (mutually exclusive with entry) */
  page?: {
    title: string;
    description?: string;
  ...
  /** Collection entry data (mutually exclusive with page) */
  entry?: {
    title: string;
    excerpt?: string;
    datePublished: string;
...
export function generateJsonLd(input: JsonLdInput): Record<string, unknown> {
  ...
  if (supportedLocales && supportedLocales.length > 1) {
    websiteNode.availableLanguage = supportedLocales.map((loc) => ({
      "@type": "Language",
      name: loc,
    }));
  }
