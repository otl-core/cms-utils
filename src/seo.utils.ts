...
export function deriveSiteUrl(
  ...
  if (siteUrlOverride) {
    ...
    const site = configs.site as Record<string, unknown> | undefined;
    const customDomains = site?.custom_domains as string[] | undefined;
    if (customDomains?.[0]) {
      url = `https://${customDomains[0]}`;
    } else {
      const subdomain = site?.subdomain as string | undefined;
